...
void Clasificador::entrena(const Datos& entr){
	if(entr.empty()){
		cerr<<"Error: datos de entrenamiento clasificador"<<endl;
		cerr<<entr.getMetaData()<<endl;
		throw runtime_error("Conjunto de entrenamiento vacio");
	}
	
	entrenamiento = entr;
	sizeAttr = entr.sizeAttr();
	sizeEntr = entr.size();
	sizePrb = 0;
...
void Clasificador::valida5f(string name, string nameAlg, vector<double> (* algorithim)(const Datos&)){
	Utils::resultado5f result; //Captura resultados parciales
	Datos dataEntr;
	Datos dataPrb;

	vector<Utils::resultado5f> resultados;
	
	for(unsigned int i = 0; i < 5; ++i){
		result.clear();
		dataEntr=FileWrapper::load5fData(name, i);
		dataPrb = FileWrapper::loadData("data/5f/"+name+"/"+name+to_string(i)+".data");
		entrena(dataEntr, dataPrb);
		
		//Tomamos el tiempo de algorithim
			Timer::startChrono();
			pesos = algorithim(dataEntr);
			result.time = Timer::elapsedTime();
		
		result.valoracion = valida5fresult();
		
		resultados.push_back(result);
		
		cout<<"conjunto data/5f/" <<name<<"/"<<name<<to_string(i)<<".data ---ok!"<<endl;
		cout<<"time: "<<result.time<<endl;
		Utils::imprimeSol(pesos);
	}
...
void Clasificador::valida5f(string name){
	Utils::resultado5f result; //Captura resultados parciales
	Datos dataEntr;
	Datos dataPrb;
...
Utils::resultado Clasificador::valida5fresult(){
	Utils::resultado result;
	result.first = validaPrueba();
	result.second = tasa_red();
	return result;
}

double Clasificador::funcionObjetivo5fPrb(){
	double result;
	result = validaPrueba() * 0.5 + tasa_red()*0.5;
	
	return result;
}

double Clasificador::funcionObjetivo5fEntr(){
	double result;
	result = validaEntr() * 0.5 + tasa_red()*0.5;
	
	return result;
}

double Clasificador::tasa_red(){
	unsigned int nMen = 0;
	...
	return (100*((double)nMen/(double)pesos.size()));
}
