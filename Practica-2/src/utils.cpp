...
namespace FileWrapper{
	...
	const Datos& load5fData(const string& name, unsigned int exclude){
		ret5fDato.clear();
		string pathfile="data/5f/"+name+"/"+name;
		for(unsigned int i = 0; i < 5; ++i){
			if(i != exclude){
				ret5fDato += loadData((pathfile+to_string(i)+".data"));
			}
		}
	...
	void guardaResultados(const vector<Utils::resultado5f>& resultados,const string& relation,const string& algorithim){
		path="data/resultados/"+algorithim+"/5f/"+relation+".result";
		openoFile();
		Utils::resultado5f mediaResult;
		ofile<<"%t T_aci T_red f_obj\n";
		for(unsigned int i = 0; i < 5; ++i){
			ofile<<resultados[i].time<<" "<<resultados[i].valoracion.first<<" "
			<<resultados[i].valoracion.second<<" "
			<<resultados[i].valoracion.first*0.5+resultados[i].valoracion.second*0.5<<"\n";
			mediaResult.suma(resultados[i]);
		}
		ofile<<"media ";
		mediaResult.media(5);
		ofile<<mediaResult.time<<" "<<mediaResult.valoracion.first<<" "
		<<mediaResult.valoracion.second<<" "<<mediaResult.valoracion.first*0.5
		+mediaResult.valoracion.second*0.5<<"\n";
