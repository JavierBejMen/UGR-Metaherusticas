...
namespace BL
{
						...
						const Datos& dataEntr)
	{
		...
		c1nn.entrena(dataEntr);
		
		mejorSol = pesos;
		c1nn.setPesos(mejorSol);
		mejorTasa = c1nn.funcionObjetivo5fEntr();
		...
		for(i = 0, j = 0; i < maxVecinos && j < maxEval; ++i, ++j)
		{
			...
			c1nn.setPesos(pesos);
			tasa = c1nn.funcionObjetivo5fEntr();
	...
	void optimiza(Clasificador* cla, Alg::Cromosoma& cromo, const unsigned int maxEval,
		...
		for(i = 0; i < maxEval && numVals < 15000; ++i)
		{
			...
			tasa = cla->funcionObjetivo5fEntr();
			++numVals;
...
void optimiza(Clasificador& cla, vector<double>& pesos, const unsigned int maxEval,
						const double radio){
		...
		cla.setPesos(mejorSol);
		mejorTasa = cla.funcionObjetivo5fEntr();
